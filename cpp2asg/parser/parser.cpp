  ...
  if (!_M_block_errors && _M_problem_count < _M_max_problem_count)
    {
      ...
      qDebug() << "ERROR in " << fileName << " at " << line << "," << column << " " << msg << endl;
...
bool Parser::parseClassSpecifier(TypeSpecifierAST *&node)
{
  ...
  ast->end_bracket = session->token_stream->cursor();
  ADVANCE_NR('}', "}");
...
bool Parser::parseAccessSpecifier(DeclarationAST *&node)
{
  ...
  ast->specs = specs;
  ast->colon = session->token_stream->cursor();
  ADVANCE(':', ":");
  UPDATE_POS(ast, start, session->token_stream->cursor());
  node = ast;
...
bool Parser::parseStatementName(StatementNameAST *&node)
{
  std::size_t start = session->token_stream->cursor();

  ADVANCE(session->token_stream->lookAhead(), "statement name");

  StatementNameAST *ast = CreateNode<StatementNameAST>(session->mempool);
...
bool Parser::parseWhileStatement(StatementAST *&node)
{
  ...
  StatementNameAST *name = 0;
  if (!parseStatementName(name))
    return false;
...
bool Parser::parseDoStatement(StatementAST *&node)
{
  ...
  std::size_t _while = session->token_stream->cursor();

  StatementNameAST *name = 0;
  if (!parseStatementName(name))
    return false;

  session->token_stream->rewind(_while);
    ...
    case '(':
      {
        ...
        ExpressionAST *expr = 0;
        parseExpression(expr);
        std::size_t p = session->token_stream->cursor();
        CHECK(')');
