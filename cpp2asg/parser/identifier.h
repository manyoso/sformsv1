...
public:
  ...
  /**
    ...
    */
  inline friend QDebug operator<< (QDebug s, const Identifier& identifier) {
    s.nospace() << identifier.toString().toLatin1().constData();
    return s.nospace();
  }
...
public:
  ...
  /**
    * kDebug() stream operator.  Writes this identifier to the debug output in a nicely formatted way.
    */
  inline friend QDebug operator<< (QDebug s, const QualifiedIdentifier& identifier) {
    s.nospace() << identifier.toString().toLatin1().constData();
    return s.nospace();
  }
