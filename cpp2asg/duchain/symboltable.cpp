...
void SymbolTable::removeDeclaration(Declaration* declaration)
{
  ...
  qWarning() << "Could not find declaration matching " << id << endl;
}
...
void SymbolTable::dumpStatistics() const
{
  ...
  qDebug() << "Definitions " << m_declarations.count() << ", Contexts " << m_contexts.count() << endl;
...
void SymbolTable::removeContext(DUContext * namedContext)
{
  ...
  qWarning() << "Could not find context matching " << id << endl;
}
