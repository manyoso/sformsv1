...
TopDUContext* ContextBuilder::buildContexts(const QUrl& url, AST *node, QList<DUContext*>* includes)
{
  m_compilingContexts = true;

  m_editor->setCurrentUrl(m_editor->findPosition(node->start_token, CppEditorIntegrator::FrontEdge).document());
  ...
  if (topLevelContext) {
    ...
    if (m_compilingContexts) {
      // To here...
      Q_ASSERT(topLevelContext->textRangePtr());
      topLevelContext->setTextRange(m_editor->createRange(m_editor->findRange(node)));
    }
    ...
    Range* range = m_editor->createRange(m_editor->findRange(node));
    topLevelContext = new TopDUContext(node, range);
  ...
  if (!m_importedParentContexts.isEmpty()) {
    qWarning() << url << " Previous parameter declaration context didn't get used??" << endl;
    DumpChain dump;
...
void ContextBuilder::visit(AST *node)
{
  if (node) {
    m_editor->setCurrentUrl(m_editor->findPosition(node->start_token, CppEditorIntegrator::FrontEdge).document());
  }
  ...
  if (m_compilingContexts && node->init_declarator && node->init_declarator->declarator && node->init_declarator->declarator->id) {
    ...
    if (functionName.count() >= 2) {
      ...
      if (classContexts.count() == 1)
        ...
        qWarning() << "Muliple class contexts for " << functionName.toString() << " - shouldn't happen!" << endl;
//         foreach (DUContext* classContext, classContexts) {
//           qDebug() << "Context " << classContext->scopeIdentifier(true) << " range " << classContext->textRange() << " in " << classContext->url() << endl;
//         }
      }
    }
  }
...
{
  if (m_compilingContexts) {
    DUContext* ret = openContextInternal(rangeNode, m_editor->findRange(rangeNode), type, identifier ? identifierForName(identifier) : QualifiedIdentifier());
...
{
  if (m_compilingContexts) {
    DUContext* ret = openContextInternal(rangeNode, m_editor->findRange(rangeNode), type, identifier);
    rangeNode->ducontext = ret;
    return ret;
...
{
  if (m_compilingContexts) {
    DUContext* ret = openContextInternal(fromRange, m_editor->findRange(fromRange, toRange), type, identifier ? identifierForName(identifier) : QualifiedIdentifier());
...
DUContext* ContextBuilder::openContextInternal(AST *node, const Range& range, DUContext::ContextType type, const QualifiedIdentifier& identifier)
{
  Q_ASSERT(m_compilingContexts);
...
//         translated = m_editor->smart()->translateFromRevision(translated);
    ...
    if (!ret) {
      ...
      ret = new DUContext(node, m_editor->createRange(range), m_contextStack.isEmpty() ? 0 : currentContext());
      ret->setType(type);
