...
//FIXME Need a replacement for kstaticdeleter
...
void DUChain::removeDocumentChain( const QUrl & document )
{
  m_chains.remove(document);
}

void DUChain::addDocumentChain( const QUrl & document, TopDUContext * chain )
{
  Q_ASSERT(chain);
  m_chains.insert(document, chain);
}

TopDUContext * DUChain::chainForDocument( const QUrl & document )
{
...
void DUChain::clear()
{
  ...
  foreach (TopDUContext* context, m_chains) {
    EditorIntegrator::releaseTopRange(context->textRangePtr());
    delete context;
  }
