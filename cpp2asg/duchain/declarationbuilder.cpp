...
TopDUContext* DeclarationBuilder::buildDeclarations(const QUrl& url, AST *node, QList<DUContext*>* includes)
{
  ...
  if (node->parameter_declaration_clause) {
    if (!m_functionDefinedStack.isEmpty() && m_functionDefinedStack.top() && node->id) {
      ...
      if (!def) {
        ...
        if (id.count() > 1) {
          Cursor pos = m_editor->findPosition(m_functionDefinedStack.top(), EditorIntegrator::FrontEdge);
          ...
          foreach (Declaration* dec, declarations) {
            ...
            abortDeclaration();
            Definition* def = new Definition(oldDec->node(), oldDec->takeRange(), currentContext());
            delete oldDec;
...
Declaration* DeclarationBuilder::openDeclaration(NameAST* name, AST* rangeNode, bool isFunction, bool isForward, bool isDefinition)
{
  ...
  AST *node = (name ? static_cast<AST*>(name) : rangeNode);
  Range newRange = m_editor->findRange(node);
...
//       translated = m_editor->smart()->translateFromRevision(translated);
  ...
  if (!declaration) {
    ...
    if (isForward) {
      declaration = new ForwardDeclaration(node, range, scope, currentContext());

    } else if (isFunction) {
      declaration = new ClassFunctionDeclaration(node, range, currentContext());
      if (!m_functionDefinedStack.isEmpty())
        declaration->setDeclarationIsDefinition(m_functionDefinedStack.top());

    } else if (scope == Declaration::ClassScope) {
      declaration = new ClassMemberDeclaration(node, range, currentContext());

    } else {
      declaration = new Declaration(node, range, scope, currentContext());
    }
