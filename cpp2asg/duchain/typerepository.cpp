...
{
  if (!input) {
    qWarning() << "Asked to register a null type." << endl;
    return input;
  }
...
AbstractType::Ptr TypeRepository::registerPointer(CppPointerType::Ptr input)
{
  ...
  if (m_pointers.contains(input->baseType())) {
    QMultiHash<AbstractType::Ptr, CppPointerType::Ptr>::Iterator it(m_pointers.constFind(input->baseType()));
...
AbstractType::Ptr TypeRepository::registerReference(CppReferenceType::Ptr input)
{
  ...
  if (m_references.contains(input->baseType())) {
    QMultiHash<AbstractType::Ptr, CppReferenceType::Ptr>::Iterator it = m_references.find(input->baseType());
...
AbstractType::Ptr TypeRepository::registerFunction(CppFunctionType::Ptr input)
{
  ...
  QMultiHash<int, CppFunctionType::Ptr>::Iterator it = m_functions.find(numElements);
...
AbstractType::Ptr TypeRepository::registerArray(ArrayType::Ptr input)
{
  ...
  if (m_arrays.contains(input->elementType())) {
    QMultiHash<AbstractType::Ptr, ArrayType::Ptr>::Iterator it = m_arrays.find(input->elementType());
