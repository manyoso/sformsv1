...
Cursor CppEditorIntegrator::findPosition( const Token & token, Edge edge ) const
{
  ...
  m_session->positionAt((edge == BackEdge) ? token.position + token.size - 1 : token.position,
                       &line, &column, &fileName);

  return Cursor(line, column, QUrl::fromLocalFile(fileName));
}

Range CppEditorIntegrator::findRange( AST * node, RangeEdge edge )
{
  Q_UNUSED(edge);
  return Range(findPosition(node->start_token, FrontEdge), findPosition(node->end_token, FrontEdge));
}

Range CppEditorIntegrator::findRange(AST* from, AST* to)
{
  return Range(findPosition(from->start_token, FrontEdge), findPosition(to->end_token, FrontEdge));
}
