...
Cursor::Cursor( int _line, int _column, const QUrl &_document)
  : m_line(_line)
  , m_column(_column)
...
void Cursor::setDocument( const QUrl &document )
{
  m_document = document;
}
...
void Cursor::cursorChangedDirectly(const Cursor& from)
{
  if (m_range) {
...
bool Cursor::atStartOfLine( ) const
{
  return m_column == 0;
}

bool Cursor::atStartOfDocument( ) const
{
  return m_line == 0 && m_column == 0;
}
