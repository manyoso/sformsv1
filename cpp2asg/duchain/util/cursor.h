...
class Cursor
{
  ...
  public:
     ...
     */
    Cursor(int line, int column, const QUrl &document = QUrl());
    ...
    /**
     * Retrieve the document url on which this cursor is situated.
     * \return document url
     */
    inline QUrl document() const { return m_document; }

    /**
     * Set the cursor's document url to \e document.
     * \param document new document url
     */
    virtual void setDocument(const QUrl &document);
...
/*                    << (cursor.document().isValid() ? cursor.document().toString(
